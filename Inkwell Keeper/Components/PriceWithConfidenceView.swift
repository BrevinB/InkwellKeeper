...
struct PriceWithConfidenceView: View {
    ...
    /// TCGplayer lowest listing, shown alongside the market price when available.
    var lowPrice: Double?
    ...
    private var inlineView: some View {
        Text("Market \(PricingService.formatPrice(price))")
            .font(.caption)
            .foregroundStyle(Color.lorcanaGold)
            .bold()
    }
    ...
    private var detailedView: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if let lowPrice {
                Text("Low \(PricingService.formatPrice(lowPrice)) on TCGplayer")
                    .font(.caption)
...
struct AsyncPriceWithConfidenceView: View {
    ...
    var body: some View {
        ...
        // Keyed to the card so a reused view refetches when the card changes —
        // the scan reveal popup swaps cards in place during rapid scanning, and
        // an unkeyed task would keep showing the first card's price.
        .task(id: card.variantAwareId) {
            ...
            let low = await pricingService.getLowPrice(for: card)

            // A newer card's task may already be running — don't overwrite its result
